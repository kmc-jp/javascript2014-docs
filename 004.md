...
- グローバル変数の宣言には関数の外で`var`を使う
- **`var`, `let`, `const`を用いずに宣言された変数**もグローバル変数となる(非推奨)
- どこからでもアクセス出来る変数
  - 実際にはグローバルオブジェクトである`Window`オブジェクトのプロパティ
  - `Window`オブジェクトを表す`Window` or `window`は省略可能
  - `hoge = 100` は `window.hoge = 100`と同等
- 昔は変数宣言に`var`を使ったが, 今はグローバル変数の宣言のみに用いる

### ローカル変数

- **関数の中で`let`を用いて宣言された変数**または仮引数
- 関数の中でのみ参照可能
...
function getValue() {
    console.log(scope);
    let scope = "Local"; // ブロック内に scope が宣言されている
    return scope;
}

console.log(getValue()); // > ReferenceError!!!
```

- 名前だけがブロックの先頭から見えるようになる
- しかし, 参照するとエラーとなる

### 関数クロージャ

 - グローバルになるケースでは変数名が衝突するかもしれない
 - 変数を関数の中に閉じ込める
...
```javascript
const obj = {}
const rand = random(); // `random()`はランダムな値を生成する関数として定義済みとする
obj[rand] = 'selected';
```
